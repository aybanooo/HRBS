...
function sendMail(int|string $reservationID, $debug = false) {
    ...
    ob_start();
    include(__D_ROOT__."/customer/email-template.php");
...
(is_numeric($_POST['rsvid'])) || die($output->setFailed('Invalid reservation ID'));

$rsvid = intval($_POST['rsvid']);
if(!sendMail($rsvid)) {
    echo $output->setFailed('Something went wrong while trying to send the email');
} else {
    echo $output->setSuccessful('Email sent');
}
?>